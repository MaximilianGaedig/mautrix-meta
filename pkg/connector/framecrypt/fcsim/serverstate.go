...
// Package fcsim builds what Messenger's SFU sends end-to-end encrypted group
// call clients, for tests of the frame-encryption module and its users.
package fcsim

import "sort"

// cw is a minimal Thrift compact-protocol writer for the server's
// E2eeServerState, which rtcsignal only decodes.
type cw struct {
	b    []byte
	last []int16
}
...
// Endpoint is one entry of E2eeServerState.endpointInfos.
...
// Config is E2eeServerStateConfig (E2eeStateSerializers; field ids
// in comments).
type Config struct {
...
// ServerState encodes an E2eeServerState in the shape of the one the
// server sent in the captured SFU call's first media update (field ids as
// decoded from that capture, see framecrypt's har_test.go): suite
// cfg.CipherSuite, protocol version 7..7, cfg's config, and endpointInfos
// keyed "<userId>:<cname>".
func ServerState(cfg Config, endpoints map[string]Endpoint) []byte {
	w := &cw{}
	w.begin()
	w.field(2, ctList)
